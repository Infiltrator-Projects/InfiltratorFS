    ...
    PCUNICODE_STRING Path, BOOLEAN Delete, BOOLEAN Permanent)
{
    ...
    if (!Delete) {
        ...
        if (Entry) {
            ...
            if (Entry && Fcb->DeletePendingCount > 0)
...
static BOOLEAN InfilfsPathDeletePending(
    INFILFS_NATIVE_FCB *Fcb, PCUNICODE_STRING Path)
{
    PLIST_ENTRY Link;
    BOOLEAN Pending = FALSE;

    if (!Fcb || !Path || !Path->Buffer)
        return FALSE;
...
static NTSTATUS InfilfsCreate(PDEVICE_OBJECT DeviceObject, PIRP Irp)
{
    ...
    if (!NT_SUCCESS(Status))
        goto complete;
    if (InfilfsPathDeletePending(Fcb, &Ccb->OpenPath)) {
...
static NTSTATUS InfilfsQueryInformation(PDEVICE_OBJECT DeviceObject, PIRP Irp)
{
    ...
    case FileStandardInformation: {
        ...
        Info->DeletePending =
            HandlePath && InfilfsPathDeletePending(Fcb, HandlePath);
        Info->Directory =
            Fcb->ObjectType == INFILFS_WIN_NATIVE_OBJECT_DIRECTORY;
        Used = sizeof(*Info);
        break;
...
static NTSTATUS InfilfsQuerySecurity(PDEVICE_OBJECT DeviceObject, PIRP Irp)
{
    ...
    Filtered = ExAllocatePool2(
        POOL_FLAG_PAGED, FullLength, INFILFS_NATIVE_REQUEST_TAG);
    if (!Filtered) {
        ExFreePoolWithTag(
            FullDescriptor, INFILFS_NATIVE_REQUEST_TAG);
        return InfilfsCompleteIrp(
            Irp, STATUS_INSUFFICIENT_RESOURCES, 0);
    }
    ...
    Status = SeQuerySecurityDescriptorInfo(
        &Information, Filtered, &Required, &ObjectDescriptor);
    ...
    __try {
        if (Required) {
            ProbeForWrite(Output, Required, sizeof(UCHAR));
            RtlCopyMemory(Output, Filtered, Required);
        }
    } __except (EXCEPTION_EXECUTE_HANDLER) {
...
out:
    if (Filtered)
        ExFreePoolWithTag(
            Filtered, INFILFS_NATIVE_REQUEST_TAG);
    if (FullDescriptor)
        ExFreePoolWithTag(
            FullDescriptor, INFILFS_NATIVE_REQUEST_TAG);
    return InfilfsCompleteIrp(
...
static NTSTATUS InfilfsSetSecurity(PDEVICE_OBJECT DeviceObject, PIRP Irp)
{
    ...
    __try {
        ...
        if (!Bytes || Bytes > INFILFS_WIN_NATIVE_IO_CHUNK)
            return InfilfsCompleteIrp(
                Irp, STATUS_BUFFER_OVERFLOW, 0);
        Captured = ExAllocatePool2(
            POOL_FLAG_PAGED, Bytes, INFILFS_NATIVE_REQUEST_TAG);
        if (!Captured)
            return InfilfsCompleteIrp(
                Irp, STATUS_INSUFFICIENT_RESOURCES, 0);
        RtlCopyMemory(Captured, Descriptor, Bytes);
    } __except (EXCEPTION_EXECUTE_HANDLER) {
        if (Captured)
            ExFreePoolWithTag(
                Captured, INFILFS_NATIVE_REQUEST_TAG);
        return InfilfsCompleteIrp(
            Irp, GetExceptionCode(), 0);
    }
...
static NTSTATUS InfilfsCleanup(PDEVICE_OBJECT DeviceObject, PIRP Irp)
{
    ...
    if (Ccb) {
        /*
         * Cleanup commits delete-on-close intent. Keep the pending path entry
         * alive until the final open handle leaves the shared FCB; only detach
         * this CCB so later Close cannot cancel or double-account the request.
         */
        Ccb->DeleteEntry = NULL;
        Ccb->DeletePending = FALSE;
    }
