...
{
    if (!volume || !native || !descriptor ||
        !IsValidSecurityDescriptor(native))
        return INFS_STATUS_INVALID_ARGUMENT;
    ...
    if (!GetSecurityDescriptorOwner(
            native, &owner, &owner_defaulted) ||
        !GetSecurityDescriptorGroup(
            native, &group, &group_defaulted) ||
        !GetSecurityDescriptorDacl(
            native, &dacl_present, &dacl, &dacl_defaulted) ||
     ...
     * evaluator is deny-by-default, so represent that meaning explicitly.
     */
    ...
    const wchar_t *native_path, int directory)
{
    ...
    DWORD error = GetNamedSecurityInfoW(
        ...
        NULL, NULL, NULL, NULL, &native);
    if (error != ERROR_SUCCESS)
        return INFS_STATUS_IO_ERROR;

    struct infs_security_descriptor descriptor;
    infs_status status = infilfs_windows_security_to_portable(
        volume, native, directory, &descriptor);
    LocalFree(native);
    if (status != INFS_STATUS_OK)
        return status;

    status = infs_set_security_descriptor(volume, path, &descriptor);
    infs_free_security_descriptor(&descriptor);
    return status;
}
    ...
    PSID owner_sid, PSID group_sid, PSID *sid_out)
{
    ...
    *sid_out = NULL;
    ...
    if (windows_id_equal(principal_id, infs_principal_group_id)) {
        if (!group_sid || !IsValidSid(group_sid))
            return INFS_STATUS_NOT_FOUND;
...
{
    if (!volume || !descriptor || !self_relative ||
        !self_relative_bytes ||
        !infs_security_descriptor_is_valid(descriptor))
        return INFS_STATUS_INVALID_ARGUMENT;

    *self_relative = NULL;
    *self_relative_bytes = 0;

    PSID owner = NULL;
    PSID group = NULL;
    PACL dacl = NULL;
    infs_status status = windows_principal_sid(
    ...
    if (!InitializeSecurityDescriptor(
        ...
        !SetSecurityDescriptorOwner(&absolute, owner, FALSE) ||
        !SetSecurityDescriptorGroup(&absolute, group, FALSE)) {
        status = INFS_STATUS_IO_ERROR;
        goto out;
    }
    if (descriptor->flags & INFS_SECURITY_DACL_PRESENT) {
        if (!SetSecurityDescriptorDacl(
                &absolute, TRUE, dacl, FALSE)) {
            status = INFS_STATUS_IO_ERROR;
            goto out;
        }
        SECURITY_DESCRIPTOR_CONTROL mask =
            SE_DACL_PROTECTED | SE_DACL_AUTO_INHERITED;
        SECURITY_DESCRIPTOR_CONTROL bits = 0;
        if (descriptor->flags & INFS_SECURITY_PROTECTED)
            bits |= SE_DACL_PROTECTED;
        if (descriptor->flags & INFS_SECURITY_AUTO_INHERIT)
            bits |= SE_DACL_AUTO_INHERITED;
        if (!SetSecurityDescriptorControl(&absolute, mask, bits)) {
            status = INFS_STATUS_IO_ERROR;
            goto out;
        }
    }
    ...
    DWORD bytes = 0;
    (void)MakeSelfRelativeSD(&absolute, NULL, &bytes);
    ...
    if (!MakeSelfRelativeSD(&absolute, buffer, &bytes)) {
        free(buffer);
        status = INFS_STATUS_IO_ERROR;
        goto out;
    }
    *self_relative = buffer;
    *self_relative_bytes = bytes;
    status = INFS_STATUS_OK;
    ...
    const wchar_t *native_path, int directory)
{
    ...
    void *self_relative = NULL;
    uint32_t self_relative_bytes = 0;
    status = infilfs_windows_security_from_portable(
        volume, &descriptor, directory,
        &self_relative, &self_relative_bytes);
    infs_free_security_descriptor(&descriptor);
    if (status != INFS_STATUS_OK)
        return status;
    ...
    if (!SetFileSecurityW(
            native_path, info,
            (PSECURITY_DESCRIPTOR)self_relative))
        status = INFS_STATUS_IO_ERROR;
    free(self_relative);
    return status;
}
