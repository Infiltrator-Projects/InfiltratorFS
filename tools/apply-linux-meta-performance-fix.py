...
s = p.read_text()
if "#include <linux/list.h>\n" not in s:
    s = one(s, "#include <linux/kernel.h>\n",
...
s = p.read_text()
init = '''static void infilfs_linux_meta_init(struct infilfs_linux_meta_header *header)
{
...
'''
s = one(s, init, helpers, "cache helpers")

get_dir = r'''static int infilfs_linux_meta_get_dir(struct super_block *sb, bool create,
...
'''
s = replace_between(s, "static int infilfs_linux_meta_get_dir(",
                    "static bool infilfs_linux_meta_directory_is_internal",
                    get_dir, "get_dir")

get_file = r'''static int infilfs_linux_meta_get_file(
...
'''
s = replace_between(s, "static int infilfs_linux_meta_get_file(",
                    "static int infilfs_linux_meta_validate_blob",
                    get_file, "get_file")

remove_locked = r'''static int infilfs_linux_meta_remove_object_locked(struct super_block *sb,
...
'''
s = replace_between(s, "static int infilfs_linux_meta_remove_object_locked(",
                    "static int infilfs_linux_meta_remove_object(",
                    remove_locked, "remove_object_locked")

store = r'''static int infilfs_linux_meta_store(struct super_block *sb,
                                    const u8 object_id[16],
...
'''
s = replace_between(s, "static int infilfs_linux_meta_store(",
...
''',
'''    ret = infilfs_ns_begin(dir->i_sb, &pending);
''', "delete forced publication")
if "infilfs_native_pending_flush_sb" in s:
    raise SystemExit("forced publication remains in Linux metadata path")
p.write_text(s)


# Free the cache on every superblock teardown.
...
'''    infilfs_quota_mount_destroy(sb);
    ...
    infilfs_linux_meta_cache_destroy(sb);
    kfree(sbi);
''', "failed mount cache destroy")
p.write_text(s)


# Existing CI already executes this policy guard, so extend it rather than
# relying on a new workflow to remember the regression.
p = Path("tests/native-small-file-scaling-policy.sh")
s = p.read_text()
if "Linux-only sidecar metadata must not perform" not in s:
    s += r'''
...
# New package version so the repair can cleanly upgrade the installed 0.18.44.
p = Path("CMakeLists.txt")
s = p.read_text()
s = one(s, "project(InfiltratorFS VERSION 0.18.44 LANGUAGES C)",
        "project(InfiltratorFS VERSION 0.18.45 LANGUAGES C)", "version")
s = one(s, "# Development qualification anchor: 0.18.44",
        "# Development qualification anchor: 0.18.45", "anchor")
p.write_text(s)

p = Path("README.md")
s = p.read_text()
s = one(s, "**Current source version:** 0.18.44 (Format 0.18)<br>",
        "**Current source version:** 0.18.45 (Format 0.18)<br>", "README hidden")
s = one(s, "**Current source:** 0.18.44  ",
        "**Current source:** 0.18.45  ", "README visible")
p.write_text(s)
