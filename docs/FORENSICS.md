...
`infilfs-forensic` is a read-only physical-block scanner introduced in implementation 0.11.0 and carried forward into current Format 0.12 media. It does not require the current namespace graph to open and does not modify, repair, mount or replay the target.
...
The scanner authenticates each recognized current-format record independently using the validation rules defined by Format 0.12, including symbolic-link, hard-link and snapshot-catalog objects:
