...
`infilfs-forensic` is a read-only physical-block scanner for current Format 0.18 media. It does not require the current namespace graph to open and does not modify, repair, mount or replay the target.
...
The scanner authenticates each recognized current-format record independently using the Format 0.18 validation contract, including symbolic-link, hard-link and snapshot-catalog objects:
