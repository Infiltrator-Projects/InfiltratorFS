...
Development 0.18.18 stores POSIX mode and numeric UID/GID compatibility metadata and supports standard Linux xattr namespaces. Those facilities are useful today but are not yet the final portable principal/ACL store.
...
Format 0.15 already reserves a `security object ID` in common attributes. That reference remains zero in the current portable format because the security-object record class and feature/version contract have not yet been standardized.
