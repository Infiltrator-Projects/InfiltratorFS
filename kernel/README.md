...
`infiltratorfs.ko` is a composite Kbuild module rather than a single giant translation unit. `infiltratorfs_core.c` owns VFS, mount and checkpoint orchestration; `infiltratorfs_cpu.c` owns the module-wide N-1 CPU execution pool/gate; `infiltratorfs_crypto.c` owns accelerated integrity hashing; `infiltratorfs_security.c` and `infiltratorfs_extension.c` own portable-security and typed-extension validation; `infiltratorfs_allocation_map.c`, `infiltratorfs_parallel_alloc.c` and `infiltratorfs_allocation_publish.c` own allocation-state loading, volatile reservations and CoW publication; `infiltratorfs_resize.c` owns mounted geometry transitions; `infiltratorfs_index_tree.c`, `infiltratorfs_extent_tree.c` and `infiltratorfs_directory_tree.c` own persistent object, extent and directory-tree operations; `infiltratorfs_read_cache.c` and `infiltratorfs_pagecache.c` own verified read caching and Linux folio/page-cache integration; `infiltratorfs_checksum_cache.c`, `infiltratorfs_checksum_store.c` and `infiltratorfs_locator_cache.c` own rebuildable/persistent integrity and locator acceleration; `infiltratorfs_linux_meta_codec.c` owns Linux sidecar codec rules; `infiltratorfs_shared_ownership.c` owns shared-range discovery and reference accounting; and `infiltratorfs_name_policy.c` owns persistent cross-platform filename policy. Shared native-driver state and the deliberately narrow cross-object API live in the private `infiltratorfs_internal.h` header.

Some already-qualified native layers are still textually composed inside the core object, including write-data paths, namespace mutation, the Linux metadata VFS/namespace adapter, quotas and defragmentation. Those include units are localized migration debt, not the module architecture and not a pattern for new features. Additional layers should move behind explicit internal APIs as independently compiled objects when their dependency boundaries are proven.
...
`kernel/Makefile` contains the canonical component and synchronization contract. `infiltratorfs_ioctl.h` carries synchronization rules with the DKMS source itself, and `tests/native-kernel-maintainability-policy.sh` requires the composite Kbuild boundary, prevents extracted allocation/index/resize/reservation/publication/read-cache/page-cache/directory-tree components from regressing into textual inclusion, blocks new nested `.inc` dependencies and macro-alias layering, and constrains growth of the remaining migration units.
