                                       ...
                                       const u8 *expected_id)
{
    ...
    if (le16_to_cpu(header->object_type) == INFILFS_OBJECT_DIRECTORY) {
        ...
        if (!infilfs_extension_object_valid(header, version, payload))
            return false;
    }
    return true;
}
    ...
    const u8 object[INFILFS_DISK_BLOCK_SIZE])
{
    ...
    if (version == INFILFS_OBJECT_VERSION_PAGED ||
        ...
        u8 *page_block;
        int ret = infilfs_extent_layout_validate(sb, object, &page_count);
    ...
    u64 *extent_logical_out, u32 *extent_blocks_out)
{
    ...
    if (version == INFILFS_OBJECT_VERSION_PAGED ||
        ...
        u8 *page_block;
        u32 lo = 0, hi;
        int ret = infilfs_extent_layout_validate(
    ...
    if (version == INFILFS_OBJECT_VERSION_CLASSIC) {
        ...
        for (i = 0; i < extent_count; ++i)
            allocated += infilfs_extent_physical_blocks(
                le32_to_cpu(extents[i].block_count),
                le32_to_cpu(extents[i].flags));
    } else if (version == INFILFS_OBJECT_VERSION_PAGED ||
               version == INFILFS_OBJECT_VERSION_TREE) {
        u32 page_count = 0, p;
        u8 *page_block;
        ...
        for (p = 0; p < page_count; ++p) {
            ...
            if (!count || count > INFILFS_EXTENTS_PER_PAGE ||
                copied > extent_count || count > extent_count - copied ||
                le32_to_cpu(page->bytes_used) != count * sizeof(*extents)) {
                ret = -EFSCORRUPTED;
                break;
            }
            extents = (const struct infilfs_extent_disk *)(page + 1);
                                     ...
                                     unsigned int flags)
{
    ...
    if (infilfs_casefold_names_enabled(dir->i_sb))
        dentry->d_op = &infilfs_casefold_dentry_ops;
...
static void infilfs_schedule_orphan_recovery(struct super_block *sb)
{
    ...
    infilfs_mod_delayed_cpu_work(&sbi->orphan_recovery_work, 1);
}
...
static int infilfs_fill_super(struct super_block *sb, struct fs_context *fc)
{
    ...
    if (infilfs_casefold_names_enabled(sb))
        sb->s_root->d_op = &infilfs_casefold_dentry_ops;
    phase_started = ktime_get_ns();
