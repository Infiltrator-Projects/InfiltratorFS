...
grep -Fq 'ENV{ID_FS_TYPE}=="infiltratorfs", ENV{ID_FS_USAGE}="", ENV{ID_FS_TYPE}=""' "$rules"
grep -Fq 'ENV{ID_FS_UUID}=""' "$rules"
...
wipe_image="$tmp/wipe.img"
cp "$image" "$wipe_image"
"$mkfs" --wipe-only "$wipe_image" >/dev/null
if "$inspect" --udev "$wipe_image" > "$tmp/wiped-probe.txt" 2>/dev/null; then
    echo 'desktop-integration: --wipe-only left an InfiltratorFS signature' >&2
    exit 1
fi
...
plain_wipe_status=$?
set -e
[[ "$plain_wipe_status" -eq 3 ]]
grep -Fq 'Target is not an InfiltratorFS volume.' "$tmp/plain-wipe.err"
