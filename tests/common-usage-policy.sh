...
mkfs1="$root/tools/mkfs.infilfs/part-01.inc"
...
grep -Fq 'set(INFILTRATR_COMMON_REQUIRED_VERSION "1.19.35")' "$cmake"
grep -Fq '7cc5de3de0e94ed2cfcff0840bbb5346eb5c9c9f' "$cmake"

# Common 1.19.35 owns theme persistence parsing/keys and deterministic ASCII
...
grep -Fq 'infiltratr_u64_multiply_checked' "$mkfs1"
grep -Fq 'pread_block_full_fd' "$mkfs1"
...
! grep -Eq '\(off_t\)\([^)]*INFS_BLOCK_SIZE' "$mkfs1" "$mkfs2" "$mkfs3" "$mkfs4"
! grep -Fq 'bitmap_blocks * INFS_BLOCK_SIZE * 8ull' "$mkfs3"
