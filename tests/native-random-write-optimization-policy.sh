...
grep -Fq 'infilfs_extent_pointer_tree_build(' "$data"
grep -Fq 'infilfs_extent_layout_defer_free(pending, old_object)' "$data"
grep -Fq 'Let the authoritative generic path promote the file to TREE.' "$data"
