...
# Boot/default fsck must route to the structural --check path. Full scrub is
# retained only as an explicit option.
grep -Fq 'INFILFS_CHECK:-/usr/bin/infilfs-inspect' "$fsck"
grep -Fq 'args=(--check "${targets[0]}")' "$fsck"
grep -Fq -- '--scrub)' "$fsck"
