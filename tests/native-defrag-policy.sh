...
grep -Fq 'inode_set_mtime_to_ts(' "$kernel"
grep -Fq 'inode_set_ctime_to_ts(' "$kernel"
grep -Fq 'old_file->attributes.modification_time' "$kernel"
grep -Fq 'old_file->attributes.change_time' "$kernel"
grep -Fq 'copy_to_user' "$kernel"
grep -Fq 'copy_from_user' "$kernel"
grep -Fq 'metrics->physical_runs = physical_runs;' "$kernel"
grep -Fq 'physical_runs' "$abi"
grep -Fq 'physical-runs=' "$tool"
grep -Fq -- '--defrag' "$tool"
