...
grep -Fq 'allocation_reservation_lists' "$state"
! grep -Fq 'unsigned long *allocation_reservations' "$state"
! grep -Fq 'allocation_reservation_bytes' "$state"
grep -Fq 'u64 token;' "$state"
grep -Fq 'allocation_reservation_serial' "$state"
grep -Fq 'struct infilfs_parallel_reservation_entry' "$allocator"
...
grep -Fq 'list_add_tail(' "$allocator"
grep -Fq 'list_del(' "$allocator"
grep -Fq 'infilfs_parallel_find_reservation_locked' "$allocator"
grep -Fq 'infilfs_parallel_range_reserved_locked' "$allocator"
! grep -Fq 'infilfs_parallel_bitmap_get' "$allocator"
! grep -Fq 'infilfs_parallel_bitmap_set' "$allocator"
! grep -Fq 'DIV_ROUND_UP_ULL(infilfs_volume_blocks(sbi), BITS_PER_LONG)' "$allocator"
! grep -Fq 'allocation_reservations' "$allocator"
! grep -Fq 'allocation_reservations' "$root/kernel/infiltratorfs_resize.c"
